package jabatan

import (
	"neosim_go/config"
	"neosim_go/internal/modules/kepegawaian/jabatan/contracts"
	"neosim_go/internal/modules/kepegawaian/jabatan/handlers"
	"neosim_go/internal/modules/kepegawaian/jabatan/repositories"
	"neosim_go/internal/modules/kepegawaian/jabatan/services"
	"neosim_go/internal/shared/cache"
	"neosim_go/internal/shared/utils"

	authContracts "neosim_go/internal/modules/auth/contracts"
	masterDepartemenContracts "neosim_go/internal/modules/master/departemen/contracts"
	rbacContracts "neosim_go/internal/modules/rbac/contracts"
	userContracts "neosim_go/internal/modules/users/contracts"

	"github.com/labstack/echo/v5"
	"gorm.io/gorm"
)

type Module struct {
	db         *gorm.DB
	handler    *handlers.KepegawaianJabatanHandler
	jwtManager *utils.JWTManager
	repo       contracts.Repository
	rbacRepo   rbacContracts.RBACRepository
}

func NewModule(
	db *gorm.DB,
	jwtManager *utils.JWTManager,
	rbacRepo rbacContracts.RBACRepository,
	authRepo authContracts.AuthRepository,
	userRepo userContracts.Repository,
	departemenRepo masterDepartemenContracts.Repository,
	cfg *config.Config,
	cacheManager *cache.Manager, // <--- Cache Manager
) *Module {
	repo := repositories.NewKepegawaianJabatanRepository(db)
	svc := services.NewKepegawaianJabatanService(repo, rbacRepo, authRepo, userRepo, departemenRepo, cfg, cacheManager)
	handler := handlers.NewKepegawaianJabatanHandler(svc, cfg)

	return &Module{
		db:         db,
		handler:    handler,
		jwtManager: jwtManager,
		repo:       repo,
		rbacRepo:   rbacRepo,
	}
}

func (m *Module) InitRoutes(e *echo.Echo) {
	RegisterRoutes(e, m.handler, m.jwtManager, m.db)
}
