...
import (
	...
	"neosim_go/internal/modules/kepegawaian/alamat/migrations"
	...
	masterAlamatRepositories "neosim_go/internal/modules/master/alamat/repositories"
...
func (r *registryModule) SetConfig(cfg *config.Config) {
	...
	if cfg.CacheMasterAlamat {
		...
		if err != nil {
			log.Printf("⚠️ Warning: Gagal koneksi ke Redis untuk cache Kepegawaian -> alamat -> Tipe: %v", err)
			r.cacheManager = cache.NewManager(nil, false, 0)
		} else {
			r.cacheManager = cache.NewManager(client, true, cfg.CacheMasterAlamatTTLDay)
			log.Println("✅ Cache Manager untukKepegawaian -> alamat -> Tipe berhasil diinisialisasi")
		}
	} else {
		r.cacheManager = cache.NewManager(nil, false, 0)
	}
...
func (r *registryModule) InitRoutes(e *echo.Echo) {
	...
	masterAlamatRepo := masterAlamatRepositories.NewMasterAlamatRepository(r.db)
	NewModule(r.db, jwtManager, r.rbacRepo, r.authRepo, userRepo, pegawaiRepo, masterAlamatRepo, r.cfg, r.cacheManager).InitRoutes(e)
}

func (r *registryModule) Models() []interface{} {
	return []interface{}{
		&models.Tipe{},
		&models.KepegawaianAlamat{},
...
func (r *registryModule) MigrateSQL(sqlDB *sql.DB) error {
	if err := migrations.MigrateKepegawaianAlamatWithSQL(sqlDB); err != nil {
		return err
	}
